...
- 如果版本检查因网络不可用或远程清单读取失败而失败，明确说明只是更新检查失败、已跳过版本检查并继续使用本地版本，除非用户明确要求先更新；不要把版本检查失败当作业务接口失败。

建议失败提示格式：

```text
tgmeng-news-skill 更新检查失败，可能是远程版本清单暂时无法访问或读取。已跳过版本检查并继续使用本地 Skill 版本，不影响本次 API 请求。
```
...
| `keywords` | string[] | 是 | 关键词数组。多个关键词项之间保持 OR；单个关键词项内支持 `+`、`＋`、`&`、`＆` 表示必须包含，`-`、`－`、`!`、`！` 表示排除。`REALTIME` 可以为空数组，`TODAY` 和 `HISTORY` 必须至少包含一个非空关键词。当前全量实时热榜请传 `keywords: []`，不要传 `[""]`，也不要省略 `keywords`。 |
...
| `offset` | integer/null | 否 | 结果偏移量，默认 `0`。配合用户确认过的 `limit` 获取后续页；下一页固定使用 `当前 offset + data.summary.returned`，例如 `limit: 100, offset: 0` 返回 `returned: 100` 后，下一页使用 `offset: 100`；负数非法。 |
| `distinct` | boolean/null | 否 | 是否按标题去重，默认 `false`。为 `true` 时，在关键词、分类、时间过滤之后、分页之前按规范化标题去重，保留当前排序中最靠前的一条；不使用 `simHash`。 |

`rootCategories` 当前已知可选值：

```text
新闻, 羊毛, 媒体, 电视, 生活, 社区, 财经, 股讯, 体育, 科技, 设计, 影音, 游戏, 健康, 教育, 期货, AI, 副业
```

调用时传上面这些值，比如 `科技`、`新闻`、`AI`。平台可能继续扩展或调整根分类，文档列表表示当前已知集合；实际可用值以接口返回的 `items[].rootCategory` 和 `rootCategories unsupported` 错误提示中的列表为准。
...
当用户想拉“当前全量热榜”或“不按关键词过滤的实时热点”时，使用 `REALTIME` 并传空数组：
...
`keywords` 是必填字段，不能省略；不要用 `[""]` 表示全量。`limit: null` 表示智能体不施加客户端条数限制，实际返回数量仍受当前实时缓存规模、密钥权限和服务端保护上限影响。调用后以 `data.summary.returned`、`data.summary.total`、`data.summary.hasMore` 为准。

## 分类过滤

可以用 `rootCategories` 缩小结果范围。根分类过滤和关键词匹配是 AND 关系；传多个根分类值时是 OR 关系。根分类值可能随平台扩展，汇总或继续筛选时应优先参考实际返回的 `items[].rootCategory`。
...
| `data.summary.hasMore` | 是否还有后续结果；为 `true` 时可保持同样条件，并用 `data.summary.offset + data.summary.returned` 作为下一页 `offset`。 |
...
默认传 `"offset": 0`。当响应里的 `data.summary.hasMore` 为 `true`，并且用户需要继续获取剩余结果时，保持其他查询条件不变，下一页固定使用：

```text
nextOffset = data.summary.offset + data.summary.returned
```

不要用 `limit` 推导下一页，也不要在 `limit: null` 时猜测页大小。示例：
